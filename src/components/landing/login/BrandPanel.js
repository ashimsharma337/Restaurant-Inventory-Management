    ...
    <section className={styles.brandPanel} aria-labelledby="brand-heading">
      ...
      <div className={styles.brandContent}>
        ...
        <p className={styles.brandDescription}>
          Elevate your kitchen operations with enterprise-grade inventory
          management designed for the modern culinary landscape.
        </p>
      </div>
      <div className={styles.trustRow}>
        <div className={styles.avatarGroup} aria-hidden="true">
          {testimonials.map(([initials, name]) => (
            <span className={styles.avatar} key={name} title={name}>
              {initials}
            </span>
          ))}
        </div>
