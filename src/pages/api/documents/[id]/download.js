...
export default async function handler(req, res) {
  ...
  try {
    const { rows } = await query(
      "SELECT object_key, original_name FROM stock_in_documents WHERE id = $1",
      [req.query.id],
    );
    if (!rows[0] || !DOCUMENT_BUCKET)
    ...
    const downloadUrl = await getSignedUrl(
      ...
      new GetObjectCommand({
        ...
        ResponseContentDisposition: `attachment; filename="${rows[0].original_name.replace(/[^a-zA-Z0-9._ -]/g, "_")}"`,
      }),
      { expiresIn: 300 },
    );
