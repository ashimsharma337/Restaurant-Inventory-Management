...
export default async function handler(req, res) {
  ...
  const { stockInReference, originalName, objectKey, contentType, fileSize } = req.body || {};
  if (!DOCUMENT_BUCKET || !stockInReference || !originalName || !objectKey || !contentType) {
    return res.status(400).json({ error: 'Document details are required' });
  }

  try {
    const object = await s3.send(new HeadObjectCommand({ Bucket: DOCUMENT_BUCKET, Key: objectKey }));
